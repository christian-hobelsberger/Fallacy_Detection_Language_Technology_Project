...
valid_model_ids=("QuantFactory/Meta-Llama-3-8B-Instruct-GGUF")
...
for i in "${!valid_model_ids[@]}"; do
    echo "$((i+1)). ${valid_model_ids[$i]}"
done

while true; do
    read -p "Enter number (default: $default_model_index): " model_index
    model_index=${model_index:-$default_model_index}
    if [[ "$model_index" =~ ^[0-9]+$ ]] && [ "$model_index" -ge 1 ] && [ "$model_index" -le "${#valid_model_ids[@]}" ]; then
        break
    else
        echo "Invalid number. Please enter a number between 1 and ${#valid_model_ids[@]}"
    fi
done

# Get selected model
modelname=${valid_model_ids[$((model_index-1))]}
